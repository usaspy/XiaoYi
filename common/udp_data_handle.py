...
def handle_0100(data,addr,_1553b):
    if data[3] == 'SYN': #设备上线报文
        ...
        try:
            ...
            period = 300
        s = device.device_id + "|" + device.device_ip + "|" + device.device_type + "|" + "ACK" + "|" + str(device.onoff) + "|" + str(period) + "|\n"

        sock_c = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        sock_c.sendto(s.encode("utf-8"), addr)
